...
private boolean isInStock(String articleTitle) {
    boolean isInStock = false;
    try {
        String query = "SELECT stock FROM article WHERE titre = ?";
        PreparedStatement statement = ds.getCnx().prepareStatement(query);
        statement.setString(1, articleTitle);
        ResultSet result = statement.executeQuery();
        if (result.next()) {
            isInStock = result.getInt(1) > 0;
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
...
/*
public Article getArticleTitle(String title) {
    // Retrieve the article with the given title from the database
    try {
        PreparedStatement statement = conn.prepareStatement("SELECT * FROM article WHERE titre = ?");
